    ...
    IERC721Enumerable
{
    ...
    uint256 internal currentIndex = 1328;
...
contract GLEBA is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor() ERC721A("GLEBA NFT", "GLEBA") {
    ...
    function getmintPay1Price(uint256 count)
        public
        view
    ...
    function getmintPay2Price(uint256 count)
        public
        view
    ...
    function getmintPay3Price(uint256 count)
        public
        view
        returns (uint256 token0)
    {
    ...
    function getmintPay4Price(uint256 count)
        public
        view
        returns (uint256 token0)
    {
    ...
    function getmintPay5Price(uint256 count)
        public
        view
        returns (uint256 token0, uint256 token1)
    {
    ...
    function mintPay1(uint256 count) public whenNotPaused nonReentrant {
        ...
        require(
            BUSD.transferFrom(_msgSender(), address(this), mintPrice),
            "Approve Token First"
        );
        ...
        if (count1 == SwapandLiquifyCount) {
            ...
            uint256[] memory returnValues = swapExactTokensForTokens(
                half,
                address(LAR)
            );
    ...
    function swapExactTokensForTokens(uint256 value, address token)
        private
        returns (uint256[] memory amounts)
    {
    ...
    function UpdateROUTER(IPancakeRouter01 _Router) public onlyOwner {
        Router = _Router;
    }

    function withdrawLar() public onlyOwner {
        LAR.transfer(owner(), LAR.balanceOf(address(this)) / 3);
    }

    function withdrawableBUSD() private view returns (uint256 amount) {
        return
            amount = BUSD.balanceOf(address(this)).sub(
                pool1.add(pool2).add(pool3).add(pool4).add(pool5)
            );
    }
    ...
    function emergencyWithdrawLar() public onlyOwner {
        LAR.transfer(owner(), LAR.balanceOf(address(this)));
    }

    function emergencyWithdrawWire() public onlyOwner {
    ...
    function UpdateBUSDReceiver(address BUSDReceiver_) public onlyOwner {
        BUSDReceiver = BUSDReceiver_;
    }

    function UpdateLpReceiver(address LpReceiver_) public onlyOwner {
        LpReceiver = LpReceiver_;
    }

    function UpdateSwapCount(uint256 swapCount_) public onlyOwner {
        SwapandLiquifyCount = swapCount_;
    }

    function UpdateSwapPercentage(uint256 swapPercent_) public onlyOwner {

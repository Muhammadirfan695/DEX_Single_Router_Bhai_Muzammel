...
contract GLEBA is ERC721A, Ownable, Pausable, ReentrancyGuard{
    ...
    {
      WIRE = IERC20(0x63e32892789d57efE8e3f464BB673A8fef50Ea43);
      LAR = IERC20(0xff38DCEB063c96675665655dAfF17E610ef4c6a8); 
      BUSD = IERC20(0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814);
      Router = IPancakeRouter01(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
      Price = IPrice(0x8eD35BDc00AdCA133f12298812C93FC8c2Da8090);
    ...
    function getmintPay1Price() public view returns(uint256 token0){
        return price1.mul(1 ether);
    }

    function getmintPay2Price() public view returns(uint256 token0, uint256 token1){
        uint256 tokenPrice = Price.one$Lar();
        token0 = price2_1.mul(1e18);
        token1 = price2_2.mul(tokenPrice);
    }

    function getmintPay3Price() public view returns(uint256 token0){
        return price3.mul(1 ether);
    }

    function getmintPay4Price() public view returns(uint256 token0){
        return price4.mul(1 ether);
    }

    function getmintPay5Price() public view returns(uint256 token0, uint256 token1){
        uint256 tokenPrice = Price.one$Wire();
        token0 = price5_1.mul(1e18);
        token1 = price5_2.mul(tokenPrice);
    }


    function mintPay1() public whenNotPaused nonReentrant{

        require(msg.sender == tx.origin," External Error ");
        uint256 mintPrice =  getmintPay1Price()  ;
        ...
        returnToken = Percentage(returnValues[2]);
        LAR.approve(address(Router), returnValues[2]);
        BUSD.approve(address(Router), half);
        addLiquiditys(returnValues[2], half);
    ...
    function mintPay2() public whenNotPaused nonReentrant{
        require(msg.sender == tx.origin," External Error ");
        (uint256 token0,uint256 token1) =  getmintPay2Price();
    ...
    function mintPay3() public whenNotPaused nonReentrant{

        require(msg.sender == tx.origin," External Error ");
        uint256 mintPrice =  getmintPay3Price();   
    ...
    function mintPay4() public whenNotPaused nonReentrant{
        require(msg.sender == tx.origin," External Error "); 
        uint256 mintPrice = getmintPay4Price();
    ...
    function mintPay5() public whenNotPaused nonReentrant{
        require(msg.sender == tx.origin," External Error ");

        (uint256 token0,uint256 token1) =  getmintPay5Price();     
        require(
